...
class Cart extends DB 
{
  ...
  {
    $this->start();
    $pass = $this->exec("INSERT INTO `orders` (`order_name`, `order_email`) VALUES (?, ?)", [$name, $email]);
  ...
  function get ($id) 
  {
    ...
    $order['items'] = $this->fetch(
      "SELECT * FROM `orders_items` LEFT JOIN `products` USING (`product_id`) WHERE `orders_items`.order_id=?", [$id], "product_id"
    );
    return $order;
  }
}
